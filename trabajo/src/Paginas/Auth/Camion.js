...
const Camion = () => {
    const [camiones, setCamiones] = useState([
        { id: 1, matricula: 'ABC123', capacidad: 5000, cargaActual: 0, consumo: 0.3 },
        { id: 2, matricula: 'DEF456', capacidad: 7000, cargaActual: 0, consumo: 0.4 },
        { id: 3, matricula: 'GHI789', capacidad: 6000, cargaActual: 0, consumo: 0.35 },
        { id: 4, matricula: 'JKL012', capacidad: 8000, cargaActual: 0, consumo: 0.5 }
    ]);

    const [activeTab, setActiveTab] = useState('lista');

    const cargarCamion = (id, carga) => {
        setCamiones(prevCamiones => prevCamiones.map(camion => {
            if (camion.id === id) {
        ...
        setCamiones(prevCamiones => prevCamiones.map(camion => {
            if (camion.id === id) {
                if (camion.cargaActual - carga >= 0) {
                    ...
                    alert('No se puede descargar más de lo que está cargado.');
                }
            }
            return camion;
        }));
            ...
            return (
                <div>
                    ...
                    <div className="form-container">
                        ...
                        <DescargarCamion camiones={camiones} descargarCamion={descargarCamion} />
                    </div>
                </div>
            );
        } else if (activeTab === 'mejor') {
            return <MejorCamion camiones={camiones} encontrarMejorCamion={encontrarMejorCamion} />;
        }
    };
    ...
    return (
        <div className="container">
            ...
            <div className="tabs">
                <button onClick={() => setActiveTab('lista')}>Lista de Camiones y Carga/Descarga</button>
